...
app.get('/sitemap.xml', async (req, res, next) => {
  try {
    ...
    courses.forEach(function(c) {
      var lastmod = c.updated_at ? String(c.updated_at).substring(0, 10) : '';
      urls.push('<url><loc>' + (process.env.APP_URL || 'https://dental-academy-production.up.railway.app') + '/courses/' + c.slug + '</loc>' + (lastmod ? '<lastmod>' + lastmod + '</lastmod>' : '') + '<priority>0.7</priority></url>');
    });
