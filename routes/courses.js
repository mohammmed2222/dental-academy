...
router.get('/create', isInstructor, async (req, res, next) => {
  try {
    ...
    return res.render('courses/create', { title: 'إنشاء كورس جديد', categories, error: null });
  } catch(err) { next(err); }
});
...
router.post('/create', isInstructor, async (req, res, next) => {
  try {
    ...
    const existing = await db.prepare('SELECT id FROM courses WHERE slug = ?').get(slug);
    if (existing) {
      slug = slug + '-' + Date.now();
    }
...
router.get('/my-courses', isInstructor, async (req, res, next) => {
  try {
    const db = getDb();
    const courses = await db.prepare(`
      SELECT c.*, 
    ...
    return res.render('courses/my-courses', { title: 'كورساتي', courses });
  } catch(err) { next(err); }
});
...
router.get('/:slug', async (req, res, next) => {
  try {
    ...
    const isEnrolled = req.session.userId ? await db.prepare(
    ...
    const exam = await db.prepare('SELECT * FROM course_exams WHERE course_id = ?').get(course.id);
    const examAttempt = exam && req.session.userId ? await db.prepare(
      "SELECT * FROM exam_attempts WHERE user_id = ? AND exam_id = ? AND passed = 1"
    ).get(req.session.userId, exam.id) : null;

    const prerequisites = await db.prepare(`
    ...
    var sections = await db.prepare('SELECT * FROM course_sections WHERE course_id = ? ORDER BY order_index ASC').all(course.id);
...
router.get('/:slug/edit', isInstructor, async (req, res, next) => {
  try {
    const db = getDb();
    ...
    const categories = await db.prepare('SELECT * FROM categories ORDER BY name').all();
    var sections = await db.prepare('SELECT * FROM course_sections WHERE course_id = ? ORDER BY order_index ASC').all(course.id);
    return res.render('courses/edit', { title: 'تعديل الكورس', course, categories, sections, error: null });
  } catch(err) { next(err); }
});

router.post('/:slug/edit', isInstructor, async (req, res, next) => {
  try {
    const db = getDb();
    const course = await db.prepare('SELECT * FROM courses WHERE slug = ? AND instructor_id = ?')
      .get(req.params.slug, req.session.userId);
...
router.post('/:slug/enroll', isAuthenticated, async (req, res, next) => {
  try {
    const db = getDb();
    ...
    if (course.price > 0) {
      const hasPaid = await db.prepare("SELECT id FROM payments WHERE user_id = ? AND course_id = ? AND status = 'paid'")
        .get(req.session.userId, course.id);
      const hasPending = await db.prepare("SELECT id FROM payments WHERE user_id = ? AND course_id = ? AND status = 'pending'")
        .get(req.session.userId, course.id);
...
router.post('/:slug/delete', isInstructor, async (req, res, next) => {
  try {
    const db = getDb();
    const course = await db.prepare('SELECT * FROM courses WHERE slug = ? AND instructor_id = ?')
      .get(req.params.slug, req.session.userId);
