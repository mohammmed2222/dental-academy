...
router.get('/', (req, res) => {
  ...
  res.render('learning-paths/list', { title: 'المسارات التعليمية', paths });
});

router.get('/create', isInstructor, (req, res) => {
  res.render('learning-paths/create', { title: 'إنشاء مسار تعليمي', error: null });
});
...
router.get('/:id', (req, res) => {
  ...
  const availableCourses = isOwner ? db.prepare(`
  ...
  res.render('learning-paths/view', {
    title: path.title,
...
router.get('/:id/edit', isInstructor, (req, res) => {
  ...
  const path = db.prepare('SELECT * FROM learning_paths WHERE id = ? AND instructor_id = ?')
  ...
  res.render('learning-paths/edit', { title: 'تعديل المسار التعليمي', path, error: null });
});
...
router.post('/:id/edit', isInstructor, (req, res) => {
  ...
  res.redirect('/learning-paths/' + pathId);
});
...
router.post('/:id/enroll', isAuthenticated, (req, res) => {
  const db = getDb();
  const pathId = parseInt(req.params.id);
  ...
  if (!existing) {
    ...
    createNotification(req.session.userId, 'learning', 'تم التسجيل في مسار تعليمي',
...
router.post('/:id/add-course', isInstructor, (req, res) => {
  const db = getDb();
  const pathId = parseInt(req.params.id);
...
router.post('/:id/remove-course/:courseId', isInstructor, (req, res) => {
  const db = getDb();
  const pathId = parseInt(req.params.id);
