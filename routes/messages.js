...
router.get('/conversation/:userId', isAuthenticated, (req, res) => {
  const db = getDb();
  const currentUserId = req.session.userId;
  const otherUserId = parseInt(req.params.userId);

  const otherUser = db.prepare('SELECT id, name, email, role, avatar FROM users WHERE id = ?').get(otherUserId);
  if (!otherUser) {
    return res.status(404).render('error', { title: 'غير موجود', message: 'المستخدم غير موجود', error: null });
  }

  db.prepare('UPDATE messages SET is_read = 1 WHERE sender_id = ? AND receiver_id = ?').run(otherUserId, currentUserId);
...
router.get('/compose', isAuthenticated, (req, res) => {
  ...
  const users = db.prepare('SELECT id, name, email, role, avatar FROM users WHERE id != ? ORDER BY name ASC').all(currentUserId);

  const preselectedUser = req.query.to ? parseInt(req.query.to) : null;
  let parentMessage = null;

  if (req.query.reply) {
    parentMessage = db.prepare('SELECT * FROM messages WHERE id = ?').get(parseInt(req.query.reply));
  }
...
router.post('/send', isAuthenticated, (req, res) => {
  ...
  db.prepare('INSERT INTO messages (sender_id, receiver_id, subject, content, parent_id) VALUES (?, ?, ?, ?, ?)')
    .run(req.session.userId, receiverId, subject, content, parentId);

  if (receiverId !== req.session.userId) {
    createNotification(receiverId, 'message', 'رسالة جديدة', req.session.userName + ' أرسل لك رسالة: ' + subject, null, null);
  }
...
router.post('/:id/read', isAuthenticated, (req, res) => {
  ...
  const message = db.prepare('SELECT * FROM messages WHERE id = ? AND receiver_id = ?').get(messageId, userId);
  if (!message) {
