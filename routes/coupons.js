...
router.post('/create', isAdmin, async (req, res, next) => {
  try {
    ...
    const existing = await db.prepare('SELECT id FROM coupons WHERE code = ?').get(code.trim());
    if (existing) {
      const coupons = await db.prepare(`
        SELECT c.*, u.name as creator_name, co.title as course_title
        FROM coupons c
...
router.post('/:id/delete', isAdmin, async (req, res, next) => {
  try {
    const db = getDb();
    await db.prepare('DELETE FROM coupons WHERE id = ?').run(parseInt(req.params.id));
