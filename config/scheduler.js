...
function startScheduler() {
  ...
  cron.schedule('0 * * * *', function() {
    (async () => {
      try {
        ...
        await db.prepare("DELETE FROM password_reset_tokens WHERE expires_at < " + require('./database').sqlNow()).run();
        if (db.raw) require('./database').saveDatabase();
  ...
  cron.schedule('0 0 * * *', function() {
    (async () => {
      try {
        var db = require('./database').getDb();
        await db.prepare("UPDATE payments SET status = 'cancelled' WHERE status = 'pending' AND created_at < " + require('./database').sqlNow('-7 days')).run();
        if (db.raw) require('./database').saveDatabase();
  ...
  cron.schedule('0 4 * * *', function() {
    (async () => {
      try {
        var db = require('./database').getDb();
        await db.prepare("DELETE FROM notifications WHERE is_read = 1 AND created_at < " + require('./database').sqlNow('-30 days')).run();
        if (db.raw) require('./database').saveDatabase();
